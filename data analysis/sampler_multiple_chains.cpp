...
// Baseline MADAGAR chain. The ordinary rho, eta, and A proposals
// jointly update their deterministic F_r/u consequences and include the
// resulting Poisson likelihood difference in the MH ratio. Its transition
// blocks otherwise match the baseline sampler.cpp implementation.
// [[Rcpp::export]]
List MADAGAR_chain(arma::vec y, arma::mat X, arma::mat Z1,
  ...
  if (initialization_regime == "original") {
    // Match the original sampler.cpp initialization.
             ...
             initialization_regime == "high_spatial_dependence") {
    rho.fill(0.85);
    if (cvrts == "adj" || cvrts == "meanadj") {
      eta = 0.05 * M;
    }
  } else if (initialization_regime == "sparse_adjacency" ||
             initialization_regime == "low_spatial_dependence") {
    rho.fill(0.20);
    if (cvrts == "adj" || cvrts == "meanadj") {
      eta = 0.85 * M;
    }
  ...
  arma::vec r = safe_zero_mean_gaussian(Vr, "initial latent-r");
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    for (int id = 0; id < nq; id++) {
      ...
      // Scalar r coordinates use the one-dimensional random-walk target
      // acceptance of 0.44.  Adaptation is restricted to burn-in so that the
      // retained chain is generated by a fixed transition kernel.
      if (iter < burn) {
    ...
    double MH_rhorho = pro_r_dens - r_dens + rho_likelihood_difference;
    for (int disease = 0; disease < q; disease++) {
      MH_rhorho +=
        pro_rhorho(disease) -
...
if (cvrts == "adj" || cvrts == "meanadj") {
    ...
    double MH_etaeta = pro_r_dens - r_dens + eta_likelihood_difference;
    ...
    // Broad numerical safeguards only.  The former lower bound at -4 could
    // bind before the joint A proposal reached its target acceptance rate.
  ...
  settings["cvrts"] = cvrts;
  settings["runs"] = runs;
  settings["burn"] = burn;
  settings["thin"] = thin;
  settings["iterations"] = iterations;
  settings["sampler"] = "support_consistent_baseline";
  settings["adaptation"] = "warmup_only";
  settings["state_consistency_tolerance"] = state_consistency_tolerance;
  settings["transition_blocks"] =
    "sampler.cpp baseline blocks; no tempering or supplementary moves";
