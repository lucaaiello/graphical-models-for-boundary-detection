rm(list = ls())
setwd("C:/Dati/Dottorato/Visiting UCLA/Spatial Disease Mapping/Bibliografia/gao_2022/simulation/Disease graph/undirected")
...
Vr_true <- readRDS("Vr_true_try.rds")
...
ARDP_joint_diff <- function(y = Y, X = X, Z = Z, Minc = Minc, W_dis = W_dis,
  ...
  rhodis <- 0.4
  rhorhodis <- log(1 + rhodis) - log(1 - rhodis)
  
  D_dis <- diag(rowSums(W_dis))
  
  L_dis <- D_dis - rhodis * W_dis
  
  R1 <- chol(1/diag(D_dis)[1]*Q[[1]])
  R2 <- chol(1/diag(D_dis)[2]*Q[[2]])
  R3 <- chol(1/diag(D_dis)[3]*Q[[3]])
  R4 <- chol(1/diag(D_dis)[4]*Q[[4]])
  
  Rblock <- as.matrix(bdiag(bdiag(R1,R2),
                            bdiag(R3,R4)))
  
  Vr <- solve(t(Rblock)%*%kronecker(L_dis,diag(n))%*%Rblock)
  ...
  R_rhorho <- diag(0.01,q)
  R_etaeta <- diag(0.01,q)
  R_rhodis <- 0.1
  ...
  xi_rhodis <- exp(s_rhodis) * R_rhodis
  ...
  for(iter in 1:runs){
    ...
    pro_R1 <- chol(1/diag(D_dis)[1]*pro_Q[[1]])
    pro_R2 <- chol(1/diag(D_dis)[2]*pro_Q[[2]])
    pro_R3 <- chol(1/diag(D_dis)[3]*pro_Q[[3]])
    pro_R4 <- chol(1/diag(D_dis)[4]*pro_Q[[4]])
    
    pro_Rblock <- as.matrix(bdiag(bdiag(pro_R1,pro_R2),
                                  bdiag(pro_R3,pro_R4)))
    
    pro_Vr <- solve(t(pro_Rblock)%*%kronecker(L_dis,diag(n))%*%pro_Rblock)
    ...
    pro_rhorhodis <- rnorm(1, rhorhodis, sqrt(xi_rhodis))
    
    pro_rhodis <- inv_trans_par(pro_rhorhodis,-1,1)
    
    pro_L_dis <- D_dis - pro_rhodis*W_dis
    
    pro_Vr <- solve(t(Rblock)%*%kronecker(pro_L_dis,diag(n))%*%Rblock)
    
    MH_rhodis <- dmvnorm(r, mean = rep(0, nq), sigma = pro_Vr, log = T) +
      pro_rhorhodis - 2*log1pexp(pro_rhorhodis) -
      dmvnorm(r, mean = rep(0, nq), sigma = Vr, log = T) - 
      rhorhodis + 2*log1pexp(rhorhodis)
    
    MH_rhodis <- min(0, MH_rhodis)
    
    if(runif(1,0,1) < exp(MH_rhodis)){
      rhodis <- pro_rhodis
      rhorhodis <- pro_rhorhodis
      L_dis <- pro_L_dis
      Vr <- pro_Vr
      acceptrhodis <- acceptrhodis + 1
    }
    
    s_rhodis <- s_rhodis + (iter+1)^(-0.7) * (exp(MH_rhodis) - 0.234)

    drhodis_2 <- as.vector(rhodis - m_rhodis)

    # Covariance
    R_rhodis <- (1-(iter+1)^(-0.7)) * R_rhodis + (iter+1)^(-0.7)*(drhodis_2%*%t(drhodis_2))

    # Mean update
    m_rhodis <- m_rhodis + (iter+1)^(-0.7) * drhodis_2

    xi_rhodis <- exp(s_rhodis) * R_rhodis

    diag(xi_rhodis) <- diag(xi_rhodis) + 1e-6 * max(diag(xi_rhodis))
...
for(seed in 1:50){
  ...
  W4 <- mcmc_samples[[seed]]$W4[order(final_perm),order(final_perm),]
