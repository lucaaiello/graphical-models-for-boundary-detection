...
List MADAGAR(const arma::vec& y, const arma::mat& X, const arma::mat& Z1,
  ...
  double r_dens = ldmvnorm(n, q, b, A, Q);
  ...
  arma::mat R_A = 1.0 * arma::eye(q * (q + 1) / 2, q * (q + 1) / 2);
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    for (int id = 0; id < nq; id++) {
      ...
      double pro_r_dens = ldmvnorm(n, q, pro_b, A, Q);
    ...
    // 
    // double pro_r_dens = ldmvnorm(n, q, b, A, pro_Q);
    // 
    ...
    pro_r_dens = ldmvnorm(n, q, pro_b, pro_A, Q);
