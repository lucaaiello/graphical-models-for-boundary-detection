...
std::vector<arma::mat> Q_new(const arma::vec& rho,
  ...
  for (int d = 0; d < q; d++) {
    ...
    arma::vec ni = arma::sum(W[d], 1);
    
    arma::mat D = arma::diagmat(ni);
    
    Q[d] = rho(d) * (D - W[d]) + (1-rho(d)) * I;
...
double ldmvnorm(int n, int q,
                const arma::vec& r,
                const std::vector<arma::mat>& Rchol,
                const arma::mat& L_dis) {
  
  double ldens = n/2* log(arma::det(L_dis));
  
  for(int d = 0; d < q; d++) {
    ldens += arma::sum(arma::log(Rchol[d].diag())) - 0.5 * L_dis(d,d) * arma::dot(Rchol[d].t() * r.subvec(d * n, (d + 1) * n - 1), Rchol[d].t() * r.subvec(d * n, (d + 1) * n - 1)); 
    for(int h = 0; h < d; h++) {
      if(L_dis(d,h) == 1.0){
        ldens -= L_dis(d,h) * arma::dot(Rchol[d] * r.subvec(d * n, (d + 1) * n - 1), Rchol[h] * r.subvec(h * n, (h + 1) * n - 1));
      }
    }
...
List MADAGAR(const arma::vec& y, const arma::mat& X, const arma::mat& Z1,
  ...
  arma::vec rho = arma::vec(q).fill(0.99);
  ...
  std::vector<arma::mat> Q = Q_new(rho, n, q, W);
  ...
  std::vector<arma::mat> Rchol(q, arma::zeros<arma::mat>(n, n));
  
  for (int d = 0; d < q; d++) {
    Rchol[d] = arma::chol(Q[d], "upper");
  }
  ...
  double r_dens = ldmvnorm(n, q, r, Rchol, L_dis);
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    for (int id = 0; id < nq; id++) {
      ...
      double pro_r_dens = ldmvnorm(n, q, pro_r, Rchol, L_dis);
    ...
    // }
    // 
    // std::vector<arma::mat> pro_Q = Q_new(pro_rho, n, q, W);
    // 
    // std::vector<arma::mat> pro_Rchol(q, arma::zeros<arma::mat>(n, n));
    // 
    // for (int d = 0; d < q; d++) {
    //   pro_Rchol[d] = arma::chol(pro_Q[d], "upper");
    // }
    // 
    // arma::vec pro_inv_Vr_diag = compute_inv_Vr_diag(n, q, pro_Q);
    // 
    // double pro_r_dens = ldmvnorm(n, q, r, pro_Rchol, L_dis);
    // 
    ...
    std::vector<arma::mat> pro_Q = Q_new(rho, n, q, pro_W); 
    ...
    pro_r_dens = ldmvnorm(n, q, r, Rchol, pro_L_dis);
