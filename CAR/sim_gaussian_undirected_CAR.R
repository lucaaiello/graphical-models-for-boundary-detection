...
Q1 <- rho[1] * (D1 - W[[1]]) + (1 - rho[1]) * diag(n)
Q2 <- rho[2] * (D2 - W[[2]]) + (1 - rho[2]) * diag(n)
Q3 <- rho[3] * (D3 - W[[3]]) + (1 - rho[3]) * diag(n)
Q4 <- rho[4] * (D4 - W[[4]]) + (1 - rho[4]) * diag(n)

R1 <- chol(1/diag(D_dis)[1]*Q1)
R2 <- chol(1/diag(D_dis)[2]*Q2)
R3 <- chol(1/diag(D_dis)[3]*Q3)
R4 <- chol(1/diag(D_dis)[4]*Q4)
...
sourceCpp('CAR/sampler_sim_gaussian_undirected_CAR_fastest.cpp')
...
for(seed in 1:100){
  ...
  filename <- paste0("CAR/runs_undirected_CAR/mcmc_samples_", seed, ".rds")
  saveRDS(mcmc_samples, file = filename)
  
}

saveRDS(W_true1, "CAR/RE_generation_undirected_CAR/W_true1.rds")
saveRDS(W_true2, "CAR/RE_generation_undirected_CAR/W_true2.rds")
saveRDS(W_true3, "CAR/RE_generation_undirected_CAR/W_true3.rds")
saveRDS(W_true4, "CAR/RE_generation_undirected_CAR/W_true4.rds")

saveRDS(X1, "CAR/RE_generation_undirected_CAR/X1.rds")
saveRDS(X2, "CAR/RE_generation_undirected_CAR/X2.rds")
saveRDS(X3, "CAR/RE_generation_undirected_CAR/X3.rds")
saveRDS(X4, "CAR/RE_generation_undirected_CAR/X4.rds")

saveRDS(phi_true1, "CAR/RE_generation_undirected_CAR/phi_true1.rds")
saveRDS(phi_true2, "CAR/RE_generation_undirected_CAR/phi_true2.rds")
saveRDS(phi_true3, "CAR/RE_generation_undirected_CAR/phi_true3.rds")
saveRDS(phi_true4, "CAR/RE_generation_undirected_CAR/phi_true4.rds")

saveRDS(Z1, "CAR/RE_generation_undirected_CAR/Z1.rds")

saveRDS(Y_list, "CAR/RE_generation_undirected_CAR/Y_list.rds")
