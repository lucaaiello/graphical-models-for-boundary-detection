...
double ldmvnorm(int n, int q,
  ...
  for(int d = 0; d < q; d++){
    ldens += 0.5 * log(arma::det(Q[d])) -
      0.5 * arma::dot(r.subvec(d * n, (d + 1) * n - 1), Q[d] * r.subvec(d * n, (d + 1) * n - 1)); // velocizzare 
  }
...
// [[Rcpp::export]]
List MCAR_indep(const arma::vec& y, const arma::mat& X, const arma::mat& Z1,
  ...
  double r_dens = ldmvnorm(n, q, r, Q);
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    for (int id = 0; id < nq; id++) {
      ...
      double pro_r_dens = ldmvnorm(n, q, pro_r, Q);
