...
`install.R`. The planned portability cleanup is to replace these uses with `sf`
equivalents while preserving current outputs.

### Runtime notes

The installation script only installs packages; it does not run the analyses.
The empirical SEER analysis in `data analysis/main.R` runs one MCMC chain with
`burn = 50000`, `runs = 10000`, and `thin = 5`, corresponding to 100000 total C++
MCMC iterations. Most simulation-generating scripts loop over 100 seeds and use
`burn = 10000`, `runs = 10000`, and `thin = 1` per seed. These scripts are
therefore long-running and should be run intentionally.

At present, some scripts assume different working directories. In particular,
`data analysis/main.R` assumes it is run from `data analysis/`, while many
simulation assessment scripts assume they are run from the repository root or from
their parent simulation folder. A later portability cleanup will standardize paths.
...
- **Table 1**  
  Generated by:  
  `Misspecified models/WAIC/WAIC_assessment.R`
...
- **Table S2**  
  Generated by:  
  `Misspecified models/rmse/rmse_assessment.R`
...
- **Figures S1**  
  Generated by:  
  `Misspecified models/WAIC/WAIC_comparison.R`
