...
colnames(rate_lung1) <- c("county", "O_count", "E_count", "standard_ratio", 
                          ...
                          "poverty", "unemployed", "black", "male", "uninsured")
...
MDAGAR <- function(y=Y, X = X, E = E, 
  ...
  list(beta = keepbeta[burn:runs,], phi=keepphi[burn:runs,], 
       ...
       rho = keeprho[burn:runs,], 
       A = keepA[burn:runs,],
       acc.beta = acceptbeta, acc.theta = accepttheta/runs, 
       acc.r = acceptr/nq/runs, acc.v = acceptv/runs, 
       acc.rho = acceptrho/runs, 
       acc.A = acceptA/runs)
}

set.seed(123)

mcmc_samples <- MDAGAR(y=Y, X = X, E = E, 
                       q = 4,Winc = Winc, Minc = Minc,
                       alpha = 1, n.atoms = 15, 
...
minESS(p = 293, alpha = .05, ess = 1607.722)
...
poverty_plot <- ggplot() +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        legend.key.size = unit(0.75,"cm"),
        legend.title = element_blank()) +
  ggtitle("Below poverty threshold proportion")
